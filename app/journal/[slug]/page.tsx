...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://claudeforge.shop/journal/${slug}` },
    openGraph: {
      title,
      description: article.description,
      type: "article",
...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Journal", item: "https://claudeforge.shop/journal" },
      { "@type": "ListItem", position: 3, name: article.category, item: `https://claudeforge.shop/journal/category/${article.category.toLowerCase()}` },
      ...
      <article className="mx-auto max-w-3xl px-5 py-16">
        <div className="mb-10">
          <nav className="text-xs text-zinc-500 mb-6 flex gap-1.5 flex-wrap">
            ...
            <Link href={`/journal/category/${article.category.toLowerCase()}`} className="hover:text-white">{article.category}</Link>
          </nav>
        ...
        <div className="prose prose-invert max-w-none prose-headings:font-semibold prose-h2:text-2xl prose-h2:mt-10 prose-h2:mb-4 prose-p:text-zinc-300 prose-p:leading-relaxed prose-strong:text-white prose-a:text-orange-400">
      ...
      {related.length > 0 && (
        <section className="mx-auto max-w-3xl px-5 pb-16">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            {related.map(a => (
              <Link key={a.slug} href={`/journal/${a.slug}`} className="group p-5 rounded-xl border border-zinc-800 hover:border-zinc-600 transition">
